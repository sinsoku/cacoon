...
public:
	...
	OAuthUrlMaker( const std::string & consumerKey, const std::string & consumerSecret );
...
private:
	...
	OAuthUrlMaker( const OAuthUrlMaker & );
	OAuthUrlMaker & operator = ( const OAuthUrlMaker & );

	std::string makeSignatureHeader();
};
